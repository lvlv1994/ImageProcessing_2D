...
// if reading in picture
   ifstream infile("pebbles.bmp", ios::in | ios::binary);
...
// writing picture
    double **fnoise=matrix(width,height);
    double **g=matrix(width,height);
    for(int jj=0;jj<3;jj++)
        for(int i = 0; i < width;i++)
            for (int j = 0; j < height ; j++){
                f[i][j] = (f[min(i+1,width-1)][j]+f[max(i-1,0)][j]+f[i][min(j+1,height-1)]+f[i][max(j-1,0)])/4;
            }
    for(int i = 0; i < width;i++)
        for (int j = 0; j < height ; j++)
        {
            f[i][j]+=0.07*gasdev(idum);
            fnoise[i][j] = f[i][j];
        }
    
    
    double fprime[2],**fprime2;
    int step = 50;
    double h = 0.05;
    double C = 0.01;
    double ex = pow(10,-9);
            ...
            for(int j = 0; j < height; j++) {
                ...
                for (int m = 0; m < 2; m++) {
                    for (int n = 0; n < 2; n++) {
                        ...
                        g[i][j] = f[i][j] + k * (C*((fprime2[0][0] + fprime2[1][1] - chaos / (gradi + h*h)) / sqrt(gradi + h*h)) - 2.0*(exp(-pow(exp(-pow(f[i][j],2)/4)/sqrt(4*pi) - fnoise[i][j],2)/4))/sqrt(4*pi));
                    }
                }
            }
        }
