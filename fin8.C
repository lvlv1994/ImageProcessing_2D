...
// writing picture
    ...
    for (i = 0; i < width; i++)
        for (j = 0; j < height; j++)
        {
            phi[i][j] = static_cast<double>(i - (width-1)/2)*static_cast<double>(i - (width-1)/2) +
            static_cast<double>(j - (height-1)/2)*static_cast<double>(j - (height-1)/2)-
            ((width-47)/2 )*((width-47)/2);
        }
    int n, r, s, maxstep = 0;
    h = 0.05;
    double RHS, **dphi2, dphi[2],df[2], gradphi2, mess, C = 100, k = h*h / 4 ,eps = 1.0e-11 ;
    dphi2 = matrix(2, 2);
   ...
   writepicture(outfile,phi,width,height);
   outfile.close();
