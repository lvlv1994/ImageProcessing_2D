...
/*    for (i = 0; i < width; i++) {
        for (j=0; j < height; j++) {
            f[i][j] = (0.5+i)*h*(0.5+j)*h;

            if(( 0.49 <= (0.5+i)*h && (0.5+i)*h<= 0.51 )&&( 0.49<=(0.5+j)*h && (0.5+j)*h <= 0.51 )) {
                g[i][j] = 1;
    ...
    for(int k = 0; k < 200; k++) {
     for (i = 0;i<500;i++)
        for(j = 0;j<500;j++) {
            if(g[i][j]==1)
            {
                fx = (f[i+1][j]-f[i-1][j])/(2*eps);
                
                fy = (f[i][j+1]-f[i][j-1])/(2*eps);
                fxx = (f[i+1][j]-2*f[i][j]+f[i-1][j])/(eps*eps);
                fyy = (f[i][j+1]-2*f[i][j]+f[i][j-1])/(eps*eps);
                fxy = (f[i+1][j+1]-f[i+1][j-1]-f[i-1][j+1]+f[i-1][j-1])/(4*eps*eps);
                e[i][j] = f[i][j]+s*((fxx+fyy)/sqrt(fx*fx+fy*fy+eps*eps)-(fx*(fxx*fx+fxy*fy)+fy*(fxy*fx+fyy*fy))
                                     /sqrt(pow(fx*fx+fy*fy+eps*eps,3)));
                cout<<fx<<endl;
                cout<<f[i+1][j]<<endl;
                cout<<i<<endl;
                cout<<j<<endl;
    ...
    for (i = 0;i<500;i++) for(j = 0;j<500;j++) {
        if ((0.5+i)*h*2-1 <= -0.5) f[i][j] = 0.25;
        if ((0.5+i)*h*2-1 > -0.5 && (0.5+i)*h*2-1 < 0.5) f[i][j] = 0.75;
        if ((0.5+i)*h*2-1 > 0.5) f[i][j] = 0.4;
        if (((0.5+i)*h*2-1 >= -0.2 && (0.5+i)*h*2-1 <= 0.2)&&((0.5+j)*h*2-1 >= -0.01 && (0.5+j)*h*2-1 <= 0.01))
            g[i][j] = 1; else
                g[i][j] = 0; }
    for (i = 0;i<500;i++) for(j = 0;j<500;j++) {
        if (g[i][j] == 1) f[i][j] = 0.5;
    }
    float fx,fy,fxx,fyy,fxy;
    double **e;
    ...
    for(int k = 0; k < 1500; k++) {
        for (i = 0;i<500;i++)
            for(j = 0;j<500;j++) {
                if(g[i][j]==1)
                {
                    fx = (float)(f[i+1][j]-f[i-1][j])/(float)(2*eps);
                    
                    fy = (float)(f[i][j+1]-f[i][j-1])/(float)(2*eps);
                    fxx = (float)(f[i+1][j]-2*f[i][j]+f[i-1][j])/(float)(eps*eps);
                    fyy = (float)(f[i][j+1]-2*f[i][j]+f[i][j-1])/(float)(eps*eps);
                    fxy = (float)(f[i+1][j+1]-f[i+1][j-1]-f[i-1][j+1]+f[i-1][j-1])/(float)(4*eps*eps);
                    e[i][j] = f[i][j]+(float)s*((fxx+fyy)/(float)sqrt(fx*fx+fy*fy+eps*eps)-(float)(fx*(fxx*fx+fxy*fy)+fy*(fxy*fx+fyy*fy))
                                           /(float)sqrt(pow(fx*fx+fy*fy+eps*eps,3)));
    ...
    e = matrix(width,height);
    for (i = 0;i<width;i++)
        for(j = 0;j<height;j++) {
        if ((double(i)/4 - i/4 == 0)||(double(j)/9 - j/9 == 0)||(double(j+1)/9 - (j+1)/9 == 0)||(double(j+2)/ 9 - (j+2)/9 == 0)||((i<=50&&i>=10)&&(j>=10&&j<=30)))
            g[i][j] = 1;
        else
            g[i][j] = 0;
    }
    ...
    for(int k = 0; k < 30000; k++)
    {
        ...
        for (i = 0;i<width;i++)
            for(j = 0;j<height;j++)
            {
                if(g[i][j]==1)
                {
                    fx = (f[min(i+1,width-1)][j]-f[max(i-1,0)][j])/(2*eps);
                    fy = (f[i][min(j+1,height-1)]-f[i][max(j-1,0)])/(2*eps);
                    fxx = (f[min(i+1,width-1)][j]-2*f[i][j]+f[max(i-1,0)][j])/(eps*eps);
                    fyy = (f[i][min(j+1,height-1)]-2*f[i][j]+f[i][max(j-1,0)])/(eps*eps);
                    fxy = (f[min(i+1,width-1)][min(j+1,height-1)]-f[min(i+1,width-1)][max(j-1,0)]-f[max(i-1,0)][min(j+1,height-1)]+f[max(i-1,0)][max(j-1,0)])/(4*eps*eps);
                    e[i][j] = f[i][j]+s*((fxx+fyy)/sqrt(fx*fx+fy*fy+eps*eps)-(fx*(fxx*fx+fxy*fy)+fy*(fxy*fx+fyy*fy))/sqrt(pow(fx*fx+fy*fy+eps*eps,3)));
                }
            }
