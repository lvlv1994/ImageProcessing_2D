...
// if reading in picture
   ifstream infile("lizard.bmp", ios::in | ios::binary);
...
/*    double **g;
    ...
    for (i = 0;i<500;i++)
        for(j = 0;j<500;j++) {
            if (((0.5+i)*h-1)*((0.5+i)*h-1)+((0.5+j)*h-1)*((0.5+j)*h-1) <= 0.25)
                g[i][j] = 1;
        else
            g[i][j] = 0;
    }
    ...
    for (int k=0; k<15000;k++) {
        for (i = 0;i<500;i++)
            for(j = 0;j<500;j++) {
                if (g[i][j] == 1) {
                    f[i][j] = f[i][j] + (f[i+1][j]-2*f[i][j]+f[i-1][j])/4 + (f[i][j+1]-2*f[i][j]+f[i][j-1])/4;
                }
            }
...
*/
    ...
    for (i = 0;i<width;i++)
        for(j = 0;j<height;j++) {
        if (i/4 - double(i)/4 == 0 || j/9 - double(j)/0 == 0 || (j+1)/9 - double(j+1)/9 == 0 || (j+2)/9 - double(j+2)/9 == 0 || ((i>=10 && i<=50) && (j>=10 && j<=30)))
            g[i][j] = 1;
        else
                g[i][j] = 0;
    }
        ...
        for (i = 0;i<width;i++) {
          ...
          for(j = 0;j<height;j++) {
              ...
              if (g[i][j] == 1) {
                ...
                new_matrix[i][j] = (f[min(i+1,width - 1)][j]+f[max(i-1,0)][j]+f[i][min(j+1,height - 1)]+f[i][max(j-1,0)])/4;
