...
int main()
{
   ...
   float a=0, b=0;
   ifstream infile("building.bmp", ios::in | ios::binary);
...
// writing picture
    ...
    for(i = 0; i < width; i++) {
        for(j = 0; j < height; j++) {
            for(int k = 0; k < N; k++) {
                if (f[i][j] > (k - 0.5)/(N - 1) && f[i][j] <= (k + 0.5)/(N-1)) {
                    f[i][j] = float(k)/(N-1);
                    ch[k]++;
                }
            }
    ...
    for(i = 0; i < width; i++) {
        for(j = 0; j < height; j++) {
            c = round(f[i][j]*(N-1));
            f[i][j] = float(ch[c]-ch[(int)a*(N-1)])/(ch[(int)b*(N-1)]-ch[(int)a*(N-1)]);
        }
    }
...
/*   cout<<f[1][1]<<endl;
    ...
    cout<<ch[N-1]<<endl;
    for(i = 0; i < N; i++) {
        g[i] = float(ch[i]-ch[0])/(ch[N-1]-ch[0]);
        cout << g[i] << endl;
    }
    for(i = 0; i < width; i++)
        for(j = 0; j < height; j++) {
    ...
    for(i = 0; i < width; i++)
        for(j = 0; j < height; j++)
            for(int n = 0; n < N; n++){
        if(f[i][j] > a+n*h && f[i][j] <= a+(n+1)*h)
        f[i][j] = (g[n+1] - g[n])/h*(f[i][j]-a-n*h)+g[n];
    }
*/
