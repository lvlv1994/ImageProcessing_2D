...
g = matrix(width,height);
    ...
    for (i = 0;i<width;i++)
        for(j = 0;j<height;j++){
            if (i/4 - double(i)/4 == 0 || j/8 - double(j)/8 == 0 || (j+1)/8 - double(j+1)/8 == 0 || (j+2)/8 -double(j+2)/8 == 0 || ((i>=10 && i<=50) && (j>=10 &&j<=30)))
                g[i][j] = 1;
            else
                g[i][j] = 0;
        }
...
*/
new_matrix = matrix(width,height);
...
for(k = 0; k < 10000; k++){
   for (i = 0;i<width;i++)
    for(j = 0;j<height;j++){
        ...
        fxy = (f[min(i+1,width-1)][min(j+1,height-1)]-f[min(i+1,width-1)][max(j-1,0)]-f[max(i-1,0)][min(j+1,height-1)]+f[max(i-1,0)][max(j-1,0)])/(4*h*h);
        if(g[i][j] == 1){
