...
/*
    newwidth = 500;
    newheight = floor(newwidth/ratio+0.5);
    ...
    double newhx = double(width)/newwidth;
    double newhy = double(height)/newheight;
    ...
    for(i = 0; i < newwidth; i++)
      for(j = 0; j < newheight; j++) {
        ...
        kx = a - fa;
        ky = b - fb;
        g[i][j] = kx*(1-ky)*f[min(fc,width-1)][min(fb,height-1)]+kx*ky*f[min(fc,width-1)]
        [min(fd,height-1)]+(1-kx)*(1-ky)*f[fa][min(fb,height-1)]+(1-kx)*ky*f[fa][min(fd,height-1)];
    }
 */
  ...
  */
    ...
    for(i = 0;i < newwidth;i++)
      for(j = 0;j < newheight;j++) {
        ...
        y = y - b + 1.5;
        for(int k = -1;k<3;k++) {
            int1[k+1] = interp(f[max(a-1,0)][max(b+k,0)],f[max(a,0)][max(b+k,0)],f[min(a+1,width-1)] [min(b+k,height-1)],f[min(a+2,width-1)][min(b+k,height-1)],x);
        }
        g[i][j] = interp(int1[0],int1[1],int1[2],int1[3],y);
    }
   ofstream outfile("result.bmp", ios::out | ios::binary);
   writepicture(outfile,g,newwidth,newheight);
   outfile.close();
