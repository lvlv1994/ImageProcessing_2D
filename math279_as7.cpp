...
// if reading in picture
   ifstream infile("bread.bmp", ios::in | ios::binary);
   ...
   infile.close();
    ...
    for(i = 0; i < width;i++)
        for (j = 0; j < height ; j++)
        {
            fnoise[i][j] = f[i][j]+0.8*gasdev(idum);
    ...
    for(int k = 0; k < 200; k++)
    {
        ...
        for (i = 0;i<width;i++)
            for(j = 0;j<height;j++)
            {
                ...
                e[i][j] = f[i][j]+s*(c*((fxx+fyy)/sqrt(fx*fx+fy*fy+eps*eps)-(fx*(fxx*fx+fxy*fy)+fy*(fxy*fx+fyy*fy))/sqrt(pow(fx*fx+fy*fy+eps*eps,3)))-2*(f[i][j]-fnoise[i][j]));
            }
        for (i = 0;i<width;i++)
            for(j = 0;j<height;j++)
            {
...
*/
    ...
    for (int kk = 0; kk < step; kk++) {
        ...
        for (i = 0; i < width; i++) {
            for(j = 0; j < height; j++) {
                fprime[0] = (f[min(i + 1, width - 1)][j] - f[max(i - 1, 0)][j]) / (2.0*h);
                fprime[1] = (f[i][min(j + 1, height - 1)] - f[i][max(j - 1, 0)]) / (2.0*h);
                fprime2[0][0] = (f[min(i + 1, width - 1)][j] - 2.0*f[i][j] + f[max(i - 1, 0)][j]) / (h*h);
                fprime2[1][1] = (f[i][min(j + 1, height - 1)] - 2.0*f[i][j] + f[i][max(j - 1, 0)]) / (h*h);
                fprime2[0][1] = (f[min(i + 1, width - 1)][min(j + 1, height - 1)] -
                                 f[min(i + 1, width - 1)][max(j - 1, 0)] -
                                 f[max(i - 1, 0)][min(j + 1, height - 1)] +
                                 f[max(i - 1, 0)][max(j - 1, 0)]) / (4.0*h*h);
                fprime2[1][0] = fprime2[0][1];
                gradi = fprime[0] * fprime[0] + fprime[1] * fprime[1];
                double chaos = 0.0;
                for (int m = 0; m < 2; m++) {
                    for (int n = 0; n < 2; n++) {
                        chaos += fprime[m] * fprime2[m][n] * fprime[n];
                        g[i][j] = f[i][j] + k * (C*((fprime2[0][0] + fprime2[1][1] - chaos / (gradi + h*h)) / sqrt(gradi + h*h)) - 2.0*(f[i][j] - fnoise[i][j]));
                    }
                }
            }
        }
