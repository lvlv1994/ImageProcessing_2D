...
// writing picture
    ...
    for (int i = 0; i < width; i++)
        for(int j = 0; j < height; j++)
            for(int k = 0; k < n; k++)
            {
                if (f[i][j] > (double(k)-0.5)/255 && f[i][j] <= (double(k)+0.5)/255)
                    ch[k]++;
            }
    ...
    for (int s = 1; s <= N; s++) {
        int k = 0;
        while ((double(ch[k]) / ch[255]) <= (double(s) / N)) {
            k++;
        }
        arr[s - 1] = double(ch[k - 1]) / ch[255];
    }
    for (int i = 0; i < width; i++) {
        for (int j = 0; j < height; j++) {
            if (f[i][j] <= arr[0]) {
    ...
    for (int i = 0; i < width; i++) {
        for (int j = 0; j < height; j++) {
            for (int s = 1; s < N; s++) {
                if (f[i][j] <= arr[s] && f[i][j] > arr[s - 1]) {
                    f[i][j] = 0.5 * (arr[s] + arr[s - 1]);
                }
            }
        }
    }
