...
double newhx,newhy,a,b;
int fa,fb,fc,fd;
double kx,ky;
int newheight, newwidth = int(width*0.8);
double ratio = 1;
//newheight = floor(newwidth/ratio+0.5);
newheight=int(height*0.8);
g = matrix(newwidth,newheight);
newhx = double(width)/newwidth;
newhy = double(height)/newheight;
...
for(i = 0; i < newwidth; i++)
  for(j = 0; j < newheight; j++){
    ...
    kx = a - fa;
    ky = b - fb;
    g[i][j] = kx*(1-ky)*f[min(fc,width-1)][min(fb,height-1)]+kx*ky*f[min(fc,width-1)][min(fd,height-1)]+(1-kx)*(1-ky)*f[fa][min(fb,height-1)]+(1-kx)*ky*f[fa][min(fd,height-1)];
}
// writing picture
